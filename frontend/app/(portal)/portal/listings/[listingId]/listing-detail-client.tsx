...
import { ChevronLeft, ImageIcon, Sparkles } from "lucide-react";
      ...
      <section className="space-y-4">
        ...
        >
          <Image
            ...
            height={640}
            priority
            className="h-[320px] w-full object-cover sm:h-[400px] lg:h-[460px] transition-transform hover:scale-105"
          />
          <div className="pointer-events-none absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/70 via-black/20 to-transparent p-6">
            <span className="inline-flex items-center gap-2 rounded-full bg-black/60 px-3 py-1 text-xs font-medium text-primary-foreground backdrop-blur">
              ...
              Click to view all {totalCount} photos
            </span>
          </div>
        </div>
        {secondary.length ? (
              ...
              return (
                ...
                >
                  <Image
                    ...
                    width={480}
                    height={320}
                    className="h-32 w-full object-cover sm:h-28 md:h-32 lg:h-36 transition-transform hover:scale-105"
                  />
                  {showOverlay ? (
            ...
            <section className="grid gap-6 lg:grid-cols-2">
              ...
              <Card>
                ...
                <CardContent className="grid gap-4 sm:grid-cols-2">
                  ...
                  {summaryDetail.neighbourhood ? (
                    <div className="rounded-lg border border-border/60 p-4">
                      <p className="text-xs uppercase text-muted-foreground">Neighbourhood</p>
                      <p className="mt-1 text-sm font-medium text-foreground">
                        {summaryDetail.neighbourhood.name}
                      </p>
                    </div>
                  ) : null}
                </CardContent>
              ...
              <Card>
                ...
                <CardContent>
                  {summaryDetail.location.latitude && summaryDetail.location.longitude ? (
                    <GoogleMapPreview
                      latitude={summaryDetail.location.latitude}
                      longitude={summaryDetail.location.longitude}
                      title={summaryDetail.location.developmentName}
                      height="280px"
                    />
                  ) : (
                    <MapPlaceholder
