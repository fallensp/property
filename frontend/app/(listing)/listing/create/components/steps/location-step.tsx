...
import { listNeighbourhoods, type Neighbourhood } from "@/lib/api/neighbourhoods";
...
export function LocationStep({ errors }: StepProps) {
  const { draft, updateLocation, updateLocationFields, updateListingType, resolvePropertyTypeIds, updateNeighbourhood } = useListingStore(
    (state) => ({
      draft: state.draft,
      ...
      updateNeighbourhood: state.updateNeighbourhood
    })
  );
  ...
  useEffect(() => {
    ...
    listNeighbourhoods()
      ...
      .catch(() => {
        // Silently fail - neighbourhoods are optional
      })
      .finally(() => {
        if (active) setIsLoadingNeighbourhoods(false);
      });
    return () => {
      active = false;
    };
        ...
        <aside className="space-y-6">
          <div className="space-y-4">
            {location.latitude && location.longitude ? (
              <GoogleMapPreview
                ...
                title={location.developmentName}
                height="320px"
              />
            ) : (
              <MapPlaceholder
          ...
          <div className="rounded-xl border border-border bg-background p-6">
            <div className="space-y-2">
              ...
              <Select
                value={draft.neighbourhoodId ? String(draft.neighbourhoodId) : "none"}
                onValueChange={(value) => {
                  updateNeighbourhood(value === "none" ? null : Number(value));
                }}
              ...
              <p className="text-xs text-muted-foreground">
                Associate this listing with a neighbourhood area.
              </p>
            </div>
          </div>
        </aside>
