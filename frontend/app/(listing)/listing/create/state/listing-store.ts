...
type ListingStoreState = {
  ...
  pendingPropertyTypeResolution: boolean;
  ...
  updatePhotoUploadStatus: (id: string, status: MediaAsset['uploadStatus']) => void;
  updatePhotoS3Data: (id: string, data: { s3Url: string; s3Id: string; thumbnailUrl?: string }) => void;
  updateListingType: (
    payload: Partial<
      Pick<
        ListingDraft,
  ...
  updateStatus: (status: ListingStatus) => void;
...
const transformToListingDraft = (listingId: string, detail: ListingDetail): ListingDraft => {
  ...
  return {
    id: listingId,
    status: (detail.summary.status as ListingStatus) ?? 'draft',
    ...
    neighbourhoodId: detail.neighbourhoodId ?? null,
    location: {
    ...
    unitDetails: {
      ...
      landArea: detail.unit.landArea ? parseNumber(detail.unit.landArea) : null,
    ...
    media: {
      ...
      virtualTours: [],
      projectPhotos: [],
      coverPhotoId: detail.gallery.length > 0 ? detail.gallery[0].id : null
    },
    platformSettings: {
      publishIProperty: true,
      publishPropertyGuru: false,
...
export const useListingStore = create<ListingStoreState>((set, get) => ({
  ...
  isUpdateMode: false,
  pendingPropertyTypeResolution: false,
  goToStep: (step) => {
  ...
  updatePhotoUploadStatus: (id, status) =>
    set((state) => ({
      draft: {
        ...state.draft,
        media: {
          ...state.draft.media,
          photos: state.draft.media.photos.map((photo) =>
            photo.id === id ? { ...photo, uploadStatus: status } : photo
          )
        },
        updatedAt: new Date().toISOString()
      }
    })),
  updatePhotoS3Data: (id, data) =>
    set((state) => ({
      draft: {
        ...state.draft,
        media: {
  ...
  updateNeighbourhood: (neighbourhoodId) =>
    set((state) => ({
      draft: {
        ...state.draft,
        neighbourhoodId,
  ...
  loadFromExisting: (listingId, data) => {
    set({
      ...
      currentStep: 'listingType',
      isUpdateMode: true,
      pendingPropertyTypeResolution: true
    });
    // Attempt immediate resolution if metadata is already cached
    get().tryResolvePropertyTypeIds();
  },
  ...
  tryResolvePropertyTypeIds: () => {
    const { draft, pendingPropertyTypeResolution } = get();
    if (!pendingPropertyTypeResolution || !draft.location?.propertyType) {
      return;
    }

    // Get metadata from property-metadata-store
    const { propertyTypes } = usePropertyMetadataStore.getState();
    if (propertyTypes.length === 0) {
      // Metadata not loaded yet - will retry when metadata loads
      return;
    }

    // Resolve the IDs and clear the pending flag
    get().resolvePropertyTypeIds(propertyTypes);
    set({ pendingPropertyTypeResolution: false });

    if (typeof window !== 'undefined') {
      console.log('Property type IDs resolved from cached metadata');
    }
  }
}));
