  ...
  const payload = buildListingPayload(draft);
    ...
    if (typeof window !== 'undefined') {
      ...
      console.log('Submission Debug:', {
        ...
        photoCount: draft.media.photos.length,
        photosWithS3Id: draft.media.photos.filter(p => p.s3Id).length,
        photosComplete: draft.media.photos.filter(p => p.uploadStatus === 'complete').length,
      });
    }
...
function buildListingPayload(draft: ListingDraft): ListingPayload {
  ...
  const attributes: Record<string, unknown> = {
    ...
    land_area_sqft: draft.unitDetails.landArea,
  ...
  // Collect image IDs from successfully uploaded photos
  const imageIds = draft.media.photos
    .filter((photo) => photo.s3Id && photo.uploadStatus === 'complete')
    .map((photo) => photo.s3Id!);
  ...
  return {
    ...
    neighbourhood_id: draft.neighbourhoodId ?? null,
