...
export function toListingSummary(listing: ApiListing): ListingSummary {
  ...
  return {
    ...
    thumbnailUrl: listing.images?.[0]?.thumbnail_url ?? listing.images?.[0]?.url ?? LISTING_THUMBNAIL_PLACEHOLDER,
    status,
...
export function toListingDetail(listing: ApiListing): ListingDetail {
  ...
  // Use real images from API, fallback to placeholder if no images
  const gallery = listing.images && listing.images.length > 0
    ? listing.images.map((img, index) => ({
        id: img.id,
        src: img.url ?? LISTING_THUMBNAIL_PLACEHOLDER,
        alt: `${listing.title ?? "Listing"} photo ${index + 1}`,
      }))
    : [{ id: "placeholder", src: LISTING_THUMBNAIL_PLACEHOLDER, alt: `${listing.title ?? "Listing"} preview` }];
  ...
  const landAreaSqft = getAttributeNumber("land_area_sqft");
  return {
    id: listing.id,
    summary,
    agent: listing.agent
      ? {
          id: listing.agent.id,
          name: listing.agent.full_name ?? summary.title,
    ...
    neighbourhoodId: listing.neighbourhood_id ?? null,
    neighbourhood: listing.neighbourhood
      ? {
          id: listing.neighbourhood.id,
          name: listing.neighbourhood.name,
          imageUrl: listing.neighbourhood.image_url,
        }
      : null,
    ...
    unit: {
      ...
      landArea: landAreaSqft > 0 ? `${landAreaSqft} sq.ft.` : undefined,
